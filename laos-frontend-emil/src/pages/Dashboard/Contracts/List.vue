          ...
          <md-card-content>
            ...
            >
              ...
              <md-table-row slot="md-table-row" slot-scope="{item}">
                <md-table-cell :md-label="$t('contracts.contract_number')" :md-tooltip="$t('contracts.contract_number')">
                  <div class="contract-num-details">
                    <md-button
                        class="md-just-icon md-simple md-success md-size-2x"
                        @click.stop.prevent='onDetails(item[indexByName(dataHeaders, "id")])'
                    >
                      <md-icon>details</md-icon>
                    </md-button>
                    {{ item[indexByName(dataHeaders, "contract_number")] }}
                  </div>
                </md-table-cell>
                <md-table-cell :md-tooltip="$t('contracts.contract_status')" :md-label="$t('contracts.contract_status')">
                  {{ getContractStatus(item[indexByName(dataHeaders, "contract_status")]) }}
                </md-table-cell>
                <md-table-cell :md-tooltip="$t('contracts.contract_title')" :md-label="$t('contracts.contract_title')">
                  {{ item[indexByName(dataHeaders, "contract_title")] }}
                </md-table-cell>
                <md-table-cell :md-tooltip="$t('contracts.project_name')" :md-label="$t('contracts.project_name')">
                  {{ item[indexByName(dataHeaders, "project_name")] }}
                </md-table-cell>
                <md-table-cell :md-tooltip="$t('contracts.contract_type')" :md-label="$t('contracts.contract_type')">
                  {{ item[indexByName(dataHeaders, "contract_type")] }}
                </md-table-cell>
                <md-table-cell :md-tooltip="$t('contracts.contractor_name')" :md-label="$t('contracts.contractor_name')">
                  {{ item[indexByName(dataHeaders, "contractor")] }}
                </md-table-cell>
                <md-table-cell :md-tooltip="$t('contracts.funding_source')" :md-label="$t('contracts.funding_source')">
                  {{ item[indexByName(dataHeaders, "funding_source")].join(", ") }}
                </md-table-cell>
                <md-table-cell :md-tooltip="$t('contracts.contract_duration')" :md-label="$t('contracts.contract_duration')" md-numeric>{{
                    item[indexByName(dataHeaders, "contract_duration")]
                  }}
                </md-table-cell>
                <md-table-cell :md-tooltip="$t('contracts.total_road_length_m')" :md-label="$t('contracts.total_road_length_m')" md-numeric>{{
                    item[indexByName(dataHeaders, "total_road_length_m")] | formatNumber
                  }}
                </md-table-cell>
                ...
                >
                  <div class='cell-actions'>
                    ...
                    <md-button
                        ...
                        @click.stop.prevent='onContractLots(item[indexByName(dataHeaders, "id")])'
                    >
                      {{ $t('button.lots') }}
                    </md-button>
                  </div>
...
export default {
  ...
  async created() {
    this.statusList = await this.$store.dispatch('LOAD_CONTRACT_STATUS_LIST')

    try {
      this.isDataLoading = true
      this.formData = await this.$store.dispatch('LOAD_CONTRACTS')
    } catch (err) {
      throw err
    } finally {
  ...
  methods: {
    getContractStatus(id) {
      ...
      const num = id > 0 && id < 5 ? id : Math.floor(Math.random() * 4 + 1);
      const elem = this.statusList.find(item => item.id === num)
      return elem.name
    },
    onAdd() {
    ...
    onEdit(contractId) {
      this.$router.push({name: "contract-upd", params: {contractId}})
    },
    onContractLots(contractId) {
      this.$router.push({name: "contract-lot-list", params: {contractId: Number(contractId)}})
    },
    onDetails(num) {
