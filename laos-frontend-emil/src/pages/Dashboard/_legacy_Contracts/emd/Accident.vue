<template>
    <div style="text-align:right; margin-top:10px;">
        ...
        <h4 class="md-title">Accident Data</h4>
        </md-toolbar>
        <md-table v-model="acdetails" id="tbl-emd" table-header-color="black" table-header-bkcolor="grey" 
        table-header-fontweight="semi-bold" table-header-gridlines="true" table-cell-gridlines="true" :md-selected-value="acdetails[selRowId]">
            <md-table-row slot="md-table-row" slot-scope="{ item }" class="iram-row" md-selectable="single">
                ...
                class="md-just-icon md-warning md-wd"
                    @click.native="handleDeleteAC(item)"
                    >
                    <md-tooltip md-direction="top">Delete Accident Data</md-tooltip>
        ...
        <md-button class="md-blue" style="height:22px;" @click.native="addNewACData()">
            <md-tooltip md-direction="top">New Accident Data</md-tooltip>
            <md-icon>add</md-icon>
        </md-button>
    </div>
</template>
...
export default {
  ...
  methods: {
    addNewACData() {
        var newIndex = this.acdetails.length + 1;
        this.acdetails.unshift({ id: newIndex.toString(), actkey: '', actname: '', actimplemval: '', comments: '' });
    ...
    deleteRowAC(item) {
        let indexToDelete = this.acdetails.findIndex(
            tableRow => tableRow.id === item.id
        );
        if (indexToDelete >= 0) {
            this.acdetails.splice(indexToDelete, 1);
        }
    },
