<template>
  <div class="EMGrid" style="position:relative">
    <div style="width:960px;">
      <div class="grid-header emd-header" style="width:100%">
        <label class="emd-label">Environmental Monitoring Data</label>
        <md-button class="md-blue" style="height:22px; width:22px; margin:0.7px 5px; float:right;" @click.native="addNewEMData()">
          <md-tooltip md-direction="top">New Environmental Monitoring Data</md-tooltip>
          <md-icon>add</md-icon>
        </md-button>
    ...
    <md-switch style="margin-left:100px; margin-top:15px;" v-model="autoedit" @change="toggle()">
      Auto-Edit (Off/On)
    </md-switch>

    <div class="inlineFilterPanel" style="display:none;background:#dddddd;padding:3px;color:black;">
      Show activities with title including <input type="text" id="txtSearch2">
      and % at least &nbsp;
      <div style="width:100px;display:inline-block;" id="pcSlider2"></div>
    </div>

    <md-snackbar :md-position="position" :md-duration="isInfinity ? Infinity : duration" :md-active.sync="showSnackbar" md-persistent>
      <span style="text-align:center;font-size:16px;line-height:1.6em;">All changes on Environment Monitoring were undone <br /> successfully!</span>
    </md-snackbar>
...
var data = [];
var columns = [
  { id: "sel", name: "#", field: "num", behavior: "select", cssClass: "cell-selection", width: 40, cannotTriggerInsert: true, resizable: false, selectable: false, excludeFromColumnPicker: true, sortable: true },
  { id: "activity", name: "Activity", field: "activity", width: 230, minWidth: 120, cssClass: "cell-activity", editor: Slick.Editors.Text, validator: requiredFieldValidator, sortable: true },
  { id: "duration", name: "Duration", field: "duration", editor: Slick.Editors.Integer, validator: durationValidator, formatter: durationFormatter, sortable: true },
  { id: "perc", defaultSortAsc: false, name: "% Complete", field: "percentComplete", width: 90, resizable: false, formatter: Slick.Formatters.PercentCompleteBar, editor: Slick.Editors.PercentComplete, sortable: true },
  ...
  { id: "comments", name: "Comments", field: "comments", width: 150, minWidth: 120, cssClass: "cell-activity", editor: Slick.Editors.Text },
  { id: "implementation", name: "Implementation", field: "implementation", width: 90, selectable: false, resizable: false },
  { id: "action", name: "Action", field: "id", width: 65, resizable: false, formatter: actionFormatter,
    cellMenu: {
      commandItems: [
        {
...
function durationValidator(newValue, args) {
  ...
  if (dataContext && (dataContext.effortDriven && (newValue < 1 || !newValue))) {
    return { valid: false, msg: 'Duration must be at least 1 day' };
  }
...
function executeCommand(e, args) {
  ...
  switch (command) {
    case "save-row":
      ...
      /*$.ajax({
          ...
          success: function (response){
            ...
            informSuccessResult('Update Activity', 'Changes on activity No. ' + dataContext.num + ' were successfully saved.');
          },
          error: function(xhr) {
    ...
    case "cancel-row":
      ...
      break;
    case "delete-row":
      confirmDelRecord(dataContext.emdid, 'the activity No. ' + dataContext.num, 'Activity', '')
        .then(function (response) {
          if (response == true) {
            let url = '/delete-activity/' + dataContext.emdid;
            /*$.ajax({
                url: url,
                type: 'POST',
                success: function (response){
                  UpdateAllTotals(grid);
                  informSuccessResult('Delete Activity', 'Activity No. ' + dataContext.num + ' was successfully deleted.');
                },
                error: function(xhr) {
                  alert(xhr.status + ': ' + xhr.responseText);
                }
...
export default {
  ...
  mounted() {
    ...
    for (var i = 0; i < 500; i++) {
      ...
      d["id"] = i;
      d["num"] = i + 1;
      d["activity"] = "Activity " + (i + 1).toString();
      d["duration"] = "5";
      d["percentComplete"] = Math.round(Math.random() * 100);
      d["start"] = "01/01/2009";
      d["finish"] = "01/05/2009";
      d["comments"] = "";
      d["implementation"] = "Satisfactory";
    }
    ...
    if (columns.length == NB_COLUMNS){
      ...
      let colIndex = [2,7]; // Activity, Comments columns
          ...
          buttons: [
            {
              ...
              command: "toggle-inputActivities",
    ...
    headerButtonsPlugin.onCommand.subscribe(function (e, args) {
      ...
      var command = args.command;
      const inputEditorColumn = {"activity": 2, "comments": 7};
      var i = inputEditorColumn[column.id];

      if (command == "toggle-inputActivities") {
    ...
    grid.onKeyDown.subscribe(function (e, args) {
      switch (args.cell) {
        case COLDURATION: // 'duration' column
          OnlyIntegers(e);
          break;
      }
    ...
    function GetRowData(rowid) {
      var rowData = {
        "id": data[rowid].id,
        "num": data[rowid].num,
        "activity": data[rowid].activity,
        "duration": data[rowid].duration,
        "percentComplete": data[rowid].percentComplete,
        "start": data[rowid].start,
        "finish": data[rowid].finish,
        "comments": data[rowid].comments,
        "implementation": data[rowid].implementation
      }
    ...
    function myFilter(item, args) {
      ...
      if (args.searchString != "" && item["activity"].indexOf(args.searchString) == -1) {
        return false;
      }
    ...
    grid.onClick.subscribe(function (e) {
      ...
      if (grid.getColumns()[cell.cell].id == "implementation") {
        if (!grid.getEditorLock().commitCurrentEdit()) {
          return;
        }

        // ===== Added By Nanthigna (04-Dec-2022) =====
        var curRow = dirtyRows.find(x => x.id === cell.row);
        if (curRow == undefined) {
          var curOldRow = {};
          curOldRow = GetRowData(cell.row);

          oldValueRows.push(curOldRow);

          dirtyRows.push(new GetRowData(cell.row));
          MarkAsDirty(cell.row);
        } else {
          var curColName = cell.column.id;

          curRow[curColName] = data[cell.row][curColName]
        }
        // ==============================================

        var states = { "Poor": "Satisfactory", "Satisfactory": "Good", "Good": "Poor" };
        data[cell.row].implementation = states[data[cell.row].implementation];
