<template>
    <div style="text-align:right; margin-top:10px;">
        ...
        <h4 class="md-title">Social Monitoring Data</h4>
        </md-toolbar>
        <md-table v-model="smdetails" id="tbl-emd" table-header-color="black" table-header-bkcolor="grey" 
        table-header-fontweight="semi-bold" table-header-gridlines="true" table-cell-gridlines="true" :md-selected-value="smdetails[selRowId]">
            <md-table-row slot="md-table-row" slot-scope="{ item }" class="iram-row" md-selectable="single">
                ...
                class="md-just-icon md-warning md-wd"
                    @click.native="handleDeletesm(item)"
                    >
                    <md-tooltip md-direction="top">Delete Social Monitoring Data</md-tooltip>
        ...
        <md-button class="md-blue" style="height:22px;" @click.native="addNewSMData()">
            <md-tooltip md-direction="top">New Social Monitoring Data</md-tooltip>
            <md-icon>add</md-icon>
        </md-button>
    </div>
</template>
...
export default {
  ...
  methods: {
    addNewSMData() {
        var newIndex = this.smdetails.length + 1;
        this.smdetails.unshift({ id: newIndex.toString(), actkey: '', actname: '', actimplemval: '', comments: '' });
    ...
    deleteRowSM(item) {
        let indexToDelete = this.smdetails.findIndex(
            tableRow => tableRow.id === item.id
        );
        if (indexToDelete >= 0) {
            this.smdetails.splice(indexToDelete, 1);
        }
    },
