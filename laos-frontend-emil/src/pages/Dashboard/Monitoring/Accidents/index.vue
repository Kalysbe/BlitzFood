      ...
      <div class='md-layout-item'>
        ...
        <md-card>
          ...
          >
            ...
            <h4 class='title'>{{ $t('label.accidents') }}</h4>
          </md-card-header>
          ...
          <md-card-content>
            ...
            >
              ...
              <md-table-row slot='md-table-row' slot-scope='{item}'>
                <md-table-cell
                    :md-label='$t(`accident.accident_date`)'
                    md-sort-by='accident_date'>
                  {{ item.accident_date }}
                </md-table-cell>
                <md-table-cell
                    :md-label='$t(`accident.accident_time`)'
                    md-sort-by='accident_time'>
                  {{ item.accident_time }}
                </md-table-cell>
                <md-table-cell
                    :md-label='$t(`accident.location`)'
                    md-sort-by='accident_location'>
                  {{ item.province_id }}
                </md-table-cell>
                <md-table-cell
                    :md-label='$t(`accident.type`)'
                    md-sort-by='accident_type'>
                  {{ item.type }}
                </md-table-cell>
                <md-table-cell
                    :md-label='$t(`accident.detail_description`)'
...
export default {
  ...
  async created() {
    ...
    try {
      this.dataList = await this.$store.dispatch('LOAD_ACCIDENTS')
    } finally {
  ...
  methods: {
    ...
    onAdd() {
      this.$router.push({name: "accident-add"})
    },
    onEdit(item) {
      this.$router.push({name: "accident-upd", params: {accidentId: item.accident_id}})
    },
    onDelete(item) {
      Swal.fire({
        title: "Are you sure?",
        ...
        if (result.value) {
          try {
            ...
            const ind = this.dataList.findIndex(row => row.accident_id === item.accident_id)
            console.log({ind})
