  ...
  ;(function(window, document, Chartist) {
    ...
    Chartist.plugins.tooltip2 = function(options) {
      ...
      return function tooltip(chart) {
        ...
        options.id = `charttooltip-${options.id ? options.id : startId}` //+ startId
        const triggerSelector = getTriggerSelector()
        const hoverClass = getDefaultTriggerClass() + '--hover'
        const tooltipElement = getTooltipElement()
        const pointValues = getPointValues()
        let hideDelayTimer

        //options.template = tooltipElement.innerHTML // Nobody knows why it here
        ...
        function init() {
          ...
          if (('line=', chart instanceof Chartist.Line)) {
        ...
        function prepareLineTooltip(e) {
          ...
          if (!targetClass === 'ct-chart-line') {
            return
          }
          // const element = e.target
          // ' ' + element.getAttribute('class') + ' ').indexOf(' ' + className + ' ') > -1;
          const boxData = e.target.getBoundingClientRect()
          ...
          const closestPointOnX = getClosestNumberFromArray(
            currentXPosition,
            pointValues
          )
         ...
         */
        function showTooltip(triggerElement, event) {
          var meta
          let value
          let textMarkup = options.template
          var seriesName
          var seriesGroups
          var seriesIndex
          var valueGroup
          ...
          setTooltipPosition(triggerElement, false, event)
        }
        ...
        function getTooltipElement() {
          const tooltipElement = document.getElementById(options.id)
         ...
         */
        function setTooltipPosition(relativeElement, ignoreClasses, event) {
          const positionData = getTooltipPosition(relativeElement, event)

          // tooltipElement.style.transform =
          //   'translate(' + positionData.left + 'px, ' + positionData.top + 'px)'
          tooltipElement.style.top = positionData.top + 'px'
          tooltipElement.style.left = positionData.left + 'px'
         ...
         */
        function getTooltipPosition(relativeElement, event) {
          const positionData = {
            alignment: 'center'
          }
          ...
          const boxData = relativeElement.getBoundingClientRect()
          const box = chart.container.getBoundingClientRect()

          if (event) {
            left =
              event.pageX - box.left - width / 2 + options.offsetCollision.x

            top = event.pageY - box.top - window.pageYOffset - height
          } else {
            left = boxData.left - width - 15 //+ options.offsetCollision.x + 20

            top = boxData.top - box.top - window.pageYOffset - height
          }
