...
$vol_sum_n_lot=0;
  ...
  if ($c = ora_logon(OP1,OP2))
  {
   ...
   //Comp
    ...
    //$name='';
    $curs = oci_parse($c, "select shortname, to_char(price,'fm999G999G990D00', 'nls_numeric_characters='', ''''') as price, volume, rowidtochar(t.rowid), date0, time0, acc.code, t.status,
    to_char(price*volume,'fm999G999G990D00', 'nls_numeric_characters='', ''''') as total
    , t.conf, to_char(profit,'fm999G999G990D00', 'nls_numeric_characters='', ''''') as profit from ls.auction_orders t, ".TS.".ts_currinstrument_arcts curr, ".TS.".tradeaccount acc where t.curr_id=curr.id and price<>-1 and t.idfirm=".$_SESSION[PORTAL.'USER']['idfirm']." and date0=".$dt." and acc.id=t.account order by shortname, price desc");
		oci_execute($curs);
   ...
   //No Comp
    ...
    while (oci_fetch($curs)) {
        ...
        echo "<tr>
         ...
         <td class='".$class."'></td>
		 <td class='".$class."'></td>
         <td class='".$class."'>".number_format($volume,2,',', ' ')."</td>
         <td class='".$class."'>".number_format($volume * NamCost, 0, ',', ' ')."</td>
         <td class='".$class."'>".$f."</td>
         </tr>";
        $name=$namerus;
    }
