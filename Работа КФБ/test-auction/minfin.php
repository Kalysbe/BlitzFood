...
$date1=date('d/m/Y');
$nProfit='0';
if (isset($_GET['nProfit'])){
    
	$nProfit=$_GET['nProfit'];

	$sql="update ls.auction_issue set profit_nocomp=".$nProfit." where curr_id=".$page." and date0=".MakeLongDate($date1);
	//echo $sql;
...
(select ls.auction_make_comp(t.price) as comp, t.volume, t.price,
                            ...
                            (select t.profit from ls.auction_deals d where d.order_id=t.rowid) as dl_doh,
                            c.shortname,t.status, 
							round((select d.profit from ls.auction_deals d where d.order_id=t.rowid),2) as profit
                          from ls.auction_orders t, ".TS.".currinstrument c
                         where t.curr_id=c.id and t.curr_id=".$page." and t.conf=1 and date0=".MakeLongDate($date1)."
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                ...
                //print("select ai.volume,ai.volume_nocomp, (select sum(volume) from ls.auction_orders ao where ao.conf=1 and ao.curr_id=ai.curr_id and ao.date0=ai.date0 and ao.price>0) as svol, (select sum(volume) from ls.auction_orders ao where ao.conf=1 and ao.curr_id=ai.curr_id and ao.date0=ai.date0 and ao.price<0) as sncvol, curr.shortname, ai.pred, ai.tot from ls.auction_issue ai, ".TS.".currinstrument curr where curr.id=ai.curr_id and curr.id=".$page."  and ai.date0=".MakeLongDate($date1));
		//echo "select ai.volume,ai.volume_nocomp, (select sum(volume) from ls.auction_orders ao where ao.curr_id=ai.curr_id and ao.date0=ai.date0 and ao.price>0) as svol, (select sum(volume) from ls.auction_orders ao where ao.curr_id=ai.curr_id and ao.date0=ai.date0 and ao.price<0) as sncvol, curr.shortname, ai.pred, ai.tot from ls.auction_issue ai, ".TS.".currinstrument curr where curr.id=ai.curr_id and curr.id=".$page."  and ai.date0=".MakeLongDate($date1);
                 ...
                 $curs77 = oci_parse($c,"select (select d.price from ls.auction_deals d where d.order_id=t.rowid) as dl_price, i.profit_nocomp_avg as profit from ls.auction_orders t, ".TS.".currinstrument c,ls.auction_issue i where t.curr_id=c.id and t.curr_id = i.curr_id and t.date0=".MakeLongDate($date1)." and i.date0=".MakeLongDate($date1)." order by shortname ASC,t.price DESC");

                 oci_execute($curs77);
                 
                 while (oci_fetch($curs77))
                  {
                    $avg_price2 = oci_result($curs77,1);
                    $avg_doh2 = oci_result($curs77,2);
                  }
   ...
   echo "
    ...
    while (oci_fetch($curs7)) {
        ...
        {
           echo 
           "<tr><td class='info'><b>Итого:</b></td><td class='info'><b>".number_format($grand_volume, 0, ',', ' ')."</b></td>
           <td class='info'>&nbsp;</td>
		     <td class='info'>&nbsp;</td>
        ...
        if ($id<=($v-$issue_vnc) && $st<2) {
          ...
          $fun='';
				...
				//echo 'dis='.$dis;
        echo "
         <td class='".$class."' id='".$id."' onclick=test('".$id."') > ";
//         onmousedown='".$reid=$id."'
//		  if (($type == 'Konkurent')){
//        if (($tp=='Конкурентный')){
          if (($tp=='Konkurent')){
    ...
    //}
     ...
     echo "
     <tr><td class='info'><b>Итого:</b></td><td class='info'><b>".number_format($grand_volume_nc, 0, ',', ' ')."</b></td>
     <td class='info'>&nbsp;</td>
     <td class='info'>&nbsp;</td>
	 <td class='info'>&nbsp;</td>
     <td class='info'>&nbsp;</td>
     <td class='info'><b>".number_format($grand_dl_volume_nc, 0, ',', ' ')."</b></td>
     <td class='info'>&nbsp;</td>
     <td class='info'><b>".number_format($grand_dl_total_nc, 2, ',', ' ')."</b></td>
     <td class='info'>&nbsp;</td>
     <td class='info'>&nbsp;</td>";
     if ($_SESSION[PORTAL.'USER']['admin']==2 && $_SESSION['MF']==0){
       echo "<td class='info'>&nbsp;</td>";
     ...
     <tr><td class='info'><b>Всего:</b></td><td class='info'><b>".number_format($grand_volume, 0, ',', ' ')."</b></td>
     <td class='info'>&nbsp;</td>
	 <td class='info'>&nbsp;</td>
     <td class='info'><b>".number_format($grand_total, 2, ',', ' ')."</b></td>
     <td class='info'>&nbsp;</td>
     ...
     echo "
	   ...
	   $kPrice = $grand_dl_total/$grand_dl_volume;
     $nPrice = $grand_dl_total_nc/$grand_dl_volume_nc;
     $issue_v=$v*70/100;
     $issue_vnc=$v-$issue_v;
     if($nPrice > 0) {
      $avg_price = $nPrice;
     ...
     //считаем количество учасников
         ...
         if ($_SESSION[PORTAL.'USER']['admin']==2 && $_SESSION['MF']==0)
         {
            echo "<br><br>
				  <table border=0>
					...
					<tr>
						<td width='30%'>Кол-во неконкурентных заявок рассчитанных МФКР: </td>
					...
					<tr>
						<td width='30%'>Доходность неконкурентных заявок рассчитанных МФКР: </td>
						<td><input class='volume-MFKR-input' type='text' size='6' style='text-align:right' name='nProfit' value='".$nProfit."' id='nProfit'></td>
					</tr>	
                  </table><br>
				  <label>
                  <button class='btn control' id='but' onclick=selid(".$page.")>Пересчитать предварительные результаты аукциона</button>
      ...
      <td>
         ...
         <tr>
            ...
            <td class='info'>
               ".number_format(myRound($value_avg_price),2,',',' ')."
            </td>
         </tr>
         <tr>
            <td class='info'>
