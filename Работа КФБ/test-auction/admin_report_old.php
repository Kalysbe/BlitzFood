   ...
   switch ($_POST['rep'])
   {
      ...
      $bank_users=" select count(*) from (
         ...
         n.nick 
          ...
          ".TS.".firm n where n.id=o.idfirm and date0=".MakeLongDate($date1)." and n.nick LIKE '%BK%' group by o.idfirm,
...
<div id='wrapper-pdf'>
 ...
 <span style='font-size:15px;'>Общая информация по аукциону:</span>
  ...
  ".TS.".currinstrument c where t.curr_id=c.id and t.date0=".MakeLongDate($date1)." order by t.price desc) 
     tt group by
      tt.comp,
      tt.price,    
          tt.profit,
      ...
      case 2: {
         ...
         if ($c = ora_logon(OP1,OP2))
         {
            ...
            else
            {
               ...
               volume, date0, time0, acc.code, f.nick, to_char(price*volume,'fm999G999G990D00', 'nls_numeric_characters='', ''''') as total,
               ( select  profit  from  ls.auction_deals d  where d.order_id = t.rowid ) as profit_2
               from ls.auction_orders t, ".TS.".ts_currinstrument_arcts curr, ".TS.".tradeaccount acc, ".TS.".firm f where t.curr_id=curr.id and t.idfirm=f.id and date0=".MakeLongDate($date1)." and acc.id=t.account ".$f." order by comp desc, price desc");
     ...
     $totalsByType[$type]['dl_total_nom'] += $dl_total_nom;
       ...
       $count = 0;
	    foreach ($totalsByType as $type => $totals) {
         ...
         // $value_dl = number_format(ceil($dl_price2 * 100)/100, 2,',',' ');
         $value_dl = number_format(myRound($dl_price2),2,',',' ');
       }
       foreach ($totalsByType as $type => $totals) {
         $count += 1;
         if($type == 'Konkurent')  {
         ...
         ";
         if($count == 1) {
            echo "<td class='info' rowspan='3' align='center'>".$value_dl."</td>
                    ...
                    if ($itogi = ora_logon("ls@".SRV,"ls"))
                    {
					  $shortNm = substr($shname,0,3);
						$curs83 = oci_parse($itogi, "select * from (select ai.volume,ai.volume_nocomp,
									  ...
									  ai.date0
									  from ls.auction_issue ai, ".TS.".currinstrument curr
									  where curr.id=ai.curr_id and curr.shortname like '". $shortNm ."%' and ai.date0<".MakeLongDate($date1) . " order by ai.date0 desc) where ROWNUM <= 1");
					  ...
					  echo "
						   ...
						   <br>
								 ...
								 <tr>
									...
									<th class='info_title'>Предыдущий аукцион " . str_replace('/', '.',MakeDate($datePred)) . "</th>
                          ...
                          $pred_v=($v*70/100);
                             ...
                             echo "
                               ...
                               <tr>
                                ...
                                <td class='info' align='right'>" . number_format($raznica2, 3, ',', ' ') . "</td>
                                </tr>
                                <tr>
                                    <td>Средневзвешенная цена (в сомах)</td>
                                    <td align='right'>".$value_dl."</td>
                                    <td align='right'></td>
                                    <td align='right'></td>
                                </tr>
                                <tr>
