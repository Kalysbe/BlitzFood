...
$grand_dl_volume_lot=0;
   ...
   (select profit from ls.auction_deals d where d.order_id=t.rowid) as dl_doh,c.shortname,t.status
...
order by c.shortname,t.price desc");
   ...
   echo "<table class='table-deals'>
    ...
    //}
     ...
     echo "<tr><td class='info'><b>Итого по лоту:</b></td><td class='info'><b>".number_format($grand_volume_lot, 0, ',', ' ')."</b></td>
                <td class='info'>&nbsp;</td>
                <td class='info'><b>".number_format($grand_total_lot, 2, ',', ' ')."</b></td>
                <td class='info'>&nbsp;</td>
                <td class='info'><b>".number_format($grand_dl_volume_lot, 0, ',', ' ')."</b></td>
                <td class='info'>&nbsp;</td>
                <td class='info'><b>".number_format($grand_dl_total_lot, 2, ',', ' ')."</b></td>
                <td class='info'>&nbsp;</td>
                </tr>
                <tr>
         </tr>
       ...
       echo "
         ...
         ";
                        ...
                        while (oci_fetch($curs8))
                        {
                          ...
                          $pred_v=($v*70/100);
                             ...
                             echo "
                               ...
                               <tr>
                                 <td class='info'><b>Предложение, сом</b></td>
                                 <td class='info' align='right'>".number_format($pred_v/100, 0, ',', ' ')."</td>
                                 <td class='info' align='right'>".number_format($pred_vnc/100, 0, ',', ' ')."</td>
                                 <td class='info' align='right'>".number_format(($pred_v+$pred_vnc)/100, 0, ',', ' ')."</td>
                               </tr>
                               <tr>
                                 <td class='info'><b>Спрос, сом</b></td>
                                 <td class='info' align='right'>".number_format($svol, 0, ',', ' ')."</td>
               ...
               <td style='padding-left: 10px' valign='top'>
                 ...
                 $qwery_str="select max(shortname),
                 min(dl_price),
                 max(dl_price),
                  ...
                  min(dl_doh),
                  max(profit) profit 
                  from ( select t.price, (select d.volume from ls.auction_deals d where d.order_id=t.rowid) as dl_volume,
                  (select round(d.price,2) from ls.auction_deals d where d.order_id=t.rowid) as dl_price, 
                  (select profit from ls.auction_deals d where d.order_id=t.rowid) as dl_doh,
                  (select profit_nocomp_avg from ls.auction_issue ai where t.curr_id = ai.curr_id) as profit,
                  c.shortname shortname, 
                 c.id as curr from ls.auction_orders t, ls.auction_issue ai,
                  ".TS.".currinstrument c where t.curr_id=c.id and t.date0=".MakeLongDate($date1)." order by shortname ASC,t.price DESC ) group by curr 
                 ...
                 <table class='table-deals'>
                   ...
                   /*if (is_resource($curs_pr))
                   {
                     ...
                     echo "
                       ...
                       <tr>
                         <td class='info'><b>Доходность к погашению, %</b></td>
                         <td class='info'>".number_format($min_doh,2,',',' ')."</td>
                         <td class='info'>".number_format($max_doh,2,',',' ')."</td>
                         <td class='info'>".number_format($avg_doh,2,',',' ')."</td>
                         </tr>
                     ";
                     }
                   //}
