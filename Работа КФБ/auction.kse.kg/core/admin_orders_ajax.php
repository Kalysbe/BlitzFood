...
$vol_sum_n_lot=0;
  ...
  if ($c = ora_logon(OP1,OP2))
  {
   //Comp
    ...
    <tr>
         <td class='info_comp' colspan='6'>
            <b>Конкурентные</b>
         </td>
         </tr>";

    $curs1 = oci_parse($c, "select shortname, to_char(price,'fm999G999G990D00', 'nls_numeric_characters='', ''''') as price,profit, volume, rowidtochar(t.rowid) as id, f.nick, t.status,acc.code, t.conf
    ...
    /*if (is_resource($curs1))
    {
        ...
        echo "
          <tr>
            <td class='info_shortname' colspan='6'>
              Лот: ".$namerus."
            </td>
          </tr>
        ";
    ...
    while (oci_fetch($curs1)) {
        ...
        $id=oci_result($curs1, 5);
        $firm=oci_result($curs1, 6);
        $status=oci_result($curs1, 7);
        $acc=oci_result($curs1, 8);
				$conf=oci_result($curs1, 9);
