...
//echo $_SESSION[PORTAL.'USER']['idfirm'];
...
$reports="<select name='report' onchange='user_report_options(this.value);'>";
...
<form method='POST'><div name='user_report_options' id='user_report_options' class='panel'><br><br></div></form>";
}
...
if (isset($_POST['rep']))
{
   ...
   {
      case 1: {
         if ($_SESSION[PORTAL.'USER']['admin']!=0){die ('Нет прав!');}
         if ($c = ora_logon(OP1,OP2))
         {
            $f=' and t.idfirm='.$_SESSION[PORTAL.'USER']['idfirm'].' ';
            if ($nocomp==0)
            {
               $f.='';
            }
      ...
      case 2: {
         ...
         if ($c = ora_logon(OP1,OP2))
         {
            ...
            else
            {
               ...
               $curs6 = oci_parse($c, "select shortname,to_char(price,'fm999G999G990D00', 'nls_numeric_characters='', ''''') as price1, volume, date0, time0, acc.code, f.nick, to_char(price*volume,'fm999G999G990D00', 'nls_numeric_characters='', ''''') as total, t.notfull, t.profit from ls.auction_deals t, ".TS.".ts_currinstrument_arcts curr, ".TS.".tradeaccount acc, ".TS.".firm f where t.curr_id=curr.id and t.idfirm=f.id and date0=".MakeLongDate($date1)." and acc.id=t.account ".$f." order by price desc");
							 oci_execute($curs6);
   ...
   echo "<table class='table-deals'>
    ...
    while (oci_fetch($curs6)) {
       ...
       $grand_volume+=$volume;
        ...
        echo "<tr>
         ...
         <td class='".$class."'>".$account."</td>
         <td class='".$class."' align='right'>".$price."</td>
		 <td class='".$class."' align='right'>".$profit."</td>
         <td class='".$class."' align='right'>".$volume."</td>
         <td class='".$class."' align='right'>".$total."</td>
         <td class='".$class."' align='right'>".$firm."</td>
         </tr>";
    ...
    //}
     ...
     echo "<tr><td class='info' colspan=6><b>Итого:</b></td><td class='info' align='right'><b>".$grand_volume."</b></td><td class='info' align='right'><b>".number_format($grand_total, 2, ',', ' ')."</b></td><td class='info'>&nbsp;</td></tr>";
...
$st2=0;
   ...
   $curs12 = oci_parse($c, "select ai.volume,ai.volume_nocomp,
   ...
   curr.shortname, ai.pred from ls.auction_issue ai, ts.currinstrument curr where curr.id=ai.curr_id and ai.date0=".MakeLongDate($date1));
	 oci_execute($curs12);
   $curs13 = oci_parse($c, "select t.mf from ls.auction_options t where date0=".MakeLongDate($date1));
	 oci_execute($curs13);
   if (oci_fetch($curs13))
   {
     $mf_agree=oci_result($curs13, 1);
   }
   else{
     $mf_agree=0;
   }
