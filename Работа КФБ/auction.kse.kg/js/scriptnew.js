window.onload=function(){
   ...
   var resultId = '';
   var wait_please = '<table width="100%" height="100%"><tr><td  align="center" valign="middle"><img src="./img/progress.gif" alt="Progress"><br>Загрузка...</td></tr></table>';
   var old_status = -1;
   ...
   {
      if (confirm("Вы действительно хотите подтвердить запись?"))
      {document.location=url;}
   }
   
   function conf_false_ord(url)
   {
      if (confirm("Вы действительно хотите отклонить запись?"))
      {document.location=url;}
   }
   
   function end_auction(url)
   {
      if (confirm("Вы действительно хотите завершить аукцион?"))
      {document.location=url;}
   }
   
   function clean_auction(url)
   {
      if (confirm("После очистки все данные по текущему аукциону будут удалены. Вы действительно хотите очистить аукцион?"))
      {document.location=url;}
   }
   
   function auction_estimate(url)
   {
      if (confirm("Вы действительно хотите произвести предварительные расчеты?"))
      {document.location=url;}
   }
   
   function auction_confirmer_go(url)
   {
      if (confirm("Завершить этап подтвержения конфирмеров?"))
      {document.location=url;}
   }
   
   function auction_reestimate(url)
   {
      if (confirm("Вы действительно хотите пересчитать предварительные расчеты?"))
      {document.location=url;}
   }
   
   function auction_estimate_go(url)
   {
      if (confirm("Вы действительно хотите подтвердить предварительные расчеты?"))
      {document.location=url;}
   }
   ...
   function recalcPrice(data,algorithm_calculation)
   {
      switch(algorithm_calculation)
      {
         case 1:
            ...
            for(var key in data) 
            {
               ...
               //alert(key + ': ' + data[key]['settlement_date']);
                  ...
                  var repayment_date = new Date(makeDateToJSDate(data[key]['repayment_date']));   	 // дата погашения
                  var coupon_annual_rate = data[key]['coupon_annual_rate']; 							 // ставка
                  var amount_ratio = data[key]['amount_ratio']; 										 // цена погашения
         ...
         case 2:
            ...
            p = (100 / (1 + (frm_profit / (360 / T ) ) ) ); 	// формула расчета ГКВ
   ...
   function report_options(val)
   {
      ...
      ro.innerHTML='<br>Загрузка...<br><br>';
