...
select tc.shortname as name, tc.namerus,ts.makedate(tc.zakdate),(case when ts.makedate(tc.zakdate) like '-' then '' else concat(', äàòà çàêðûòèÿ ',to_char(ts.makedate(tc.zakdate))) end) as dt,
 ...
 where b is not null or s is not null";
    ...
    $sql = "DELETE FROM `mod_tradestat` WHERE 1=1";
    mysql_query($sql);
	...
	$sql = "INSERT INTO `mod_tradestat` (
			...
			'" . str_replace(",", ".", $buf["STOTAL"]) . "',
			'" . str_replace(",", ".", $buf["TOTALCNG"]) . "',
			'" . $TotalChange . "',
			'" . str_replace(",",".",$buf["SRAZM"]) . "',
			'" . str_replace(",",".",$buf["RAZMCNG"]) . "',
			'" . $PrimaryChange . "',
			'" . str_replace(",",".",$buf["SSECOND"]) . "',
			'" . str_replace(",",".",$buf["SECONDCNG"]) . "',
			'" . $SecondaryChange . "',
			'" . str_replace(",",".",$buf["SLIST"]) . "',
			'" . str_replace(",",".",$buf["LISTCNG"]) . "',
			'" . $ListingChange . "',
			'" . str_replace(",",".",$buf["SUNLIST"]) . "',
			'" . str_replace(",",".",$buf["UNLISTCNG"]) . "',
			'" . $NonListingChange . "'
		)";
mysql_query($sql);
