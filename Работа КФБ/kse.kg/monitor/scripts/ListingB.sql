select '<a target=_blank class="link" href=listing/' ||
(select min(tt.shortname) from ls.extra_security tt where tt.enterprise=t.id) || '.shtml>' ||
t.name  || '</a>' as name,
...
'<a href=listing\' || (select min(tt.shortname) from ls.extra_security tt where tt.enterprise=t.id) || '.pdf>
<img src="img\pdf.gif" border=0></a>' as doc
from ls.extra_enterprise t
where t.ind=1 and WWW.GETLSTYPE(t.name)='Категория В';