...
class Page {
	...
	{
		//print_r("conn=%1",MainClass::getSingleton()->getDbConnection());
		return MainClass::getSingleton()->getDbConnection();
	}
		
    public function __construct($pageUname = "", $pageId = 0, $isDefault = false) {
		
		$this->dbConnect = MainClass::getSingleton()->getDbConnection(); // возможно заменить на функцию getDbConnect() 
    ...
    private function processPage() {
        ...
        if ($this->Status == PAGE_STATUS_HANDLED) {
                $sql = "SELECT * FROM `page_extensions` WHERE (`page_id`='" . $this->Id . "')";
                $result = mysqli_query($this->dbConnect, $sql);
                $rows = $result->num_rows;
                if ($rows != 0) {
                    for ($i = 0; $i < $rows; $i++) {
            ...
            if (file_exists(trim($this->Text))) {

                require_once(MainClass::getSingleton()->getSiteVar("%HOME%") . "/" . trim($this->Text));
    ...
    private function loadHandlers() {
        ...
        if (!$this->isHandlersLoaded) {
            $___sql = "SELECT `handler` FROM `page_handlers` WHERE (`page_id`='" . $this->Id . "')";
            $___result = mysqli_query($this->dbConnect , $___sql);
            $___rows = $___result->num_rows;
            for ($___i = 0; $___i < $___rows; $___i++) {
                mysqli_data_seek($___result, $___i);
                $___obj = mysqli_fetch_object($___result);
                
                if (file_exists('./page_handlers/' . $___obj->handler)) {
                    include_once('./page_handlers/' . $___obj->handler);
                } else {
        ...
        foreach ($fullPatternsArray as $index => $fullPattern) {
            ...
            if (strpos($moduleContentArray[$index], "|") > 0) {
                ...
                if ($rows != 1)
                    ...
                    $obj = mysqli_fetch_object($result);
                    mysqli_query($this->dbConnect, "INSERT INTO `modules_applied` 
                    (`mid`, `page_id`, `phrase`, `status`) VALUES ('" .
                    $obj->mid . "', '" . $this->Id . "', '" . $fullPatternsArray[$index] . "', '1') ");
                    $moduleId = mysqli_insert_id();

                    foreach ($parameters as $parameter) {
                        if (strpos($parameter, "=") > 0) {
                            $pair = explode("=", $parameter);
                            mysqli_query($this->dbConnect, "INSERT INTO `modules_parameters`
    ...
    private function removeModules() {
        ...
        for ($i = 0; $i < $rows; $i++) {
            mysqli_data_seek($result, $i);
            $obj = mysqli_fetch_object($result);
            mysqli_query($this->dbConnect , "DELETE FROM `modules_parameters` WHERE (`mappid`='" .
                $obj->mappid . "')");
        }
        mysqli_query($this->dbConnect, "DELETE FROM `modules_applied` WHERE (`page_id`='" .
            $this->Id . "')");
    }
