...
class MainClass {
    ...
    public function checkUserLogin() {
        ...
        if (isset($_SESSION["LOGGED"])) {
            if ($_SESSION["LOGGED"] == true) {
				...
				$sql = "SELECT `uid` FROM `sessions` WHERE (`hash`='" . $_SESSION["HASH"] . "')";
    ...
    public function getRows($table, $criteria) {
        $result = mysqli_query("SELECT COUNT(1) FROM `" . $table . "` WHERE (" . $criteria . ")");
    ...
    public function loadSetting() {
        $result = mysqli_query("SELECT `value` FROM `settings` WHERE (`name`='" . $name . "')");
        $rows = mysqli_num_rows($result);
        if ($rows != 0)
            $obj = mysqli_fetch_object($result);
        else return null;
...
if (!isset($_SESSION["LANGUAGE"])) {
    $_SESSION["LANGUAGE"] = MainClass::getSingleton()->getSiteVar("DEFAULT_LANGUAGE");
}

// Redirect to default page if no page was requested
if (!MainClass::getSingleton()->buildUriPath()) {
    MainClass::getSingleton()->setLanguage($_SESSION["LANGUAGE"]);
...
$mime_types = array(
    ...
    "image/png"=>"icon_image.png",
    "image/jpeg"=>"icon_image.png",
    "image/bmp"=>"icon_image.png",
);

function build_datetime_mysql($datetime) {
...
function build_date_mysql($date) {
...
function buildRequestFromPost() {
    $link = MainClass::getSingleton()->getDbConnection();
	$array = array();
    foreach ($_POST as $k=>$v) {
        if ((trim($k) != "")&&(trim($v) != ""))
            $array[trim($k)] = mysqli_real_escape_string($link, trim($v));
    }
    return $array;
}
