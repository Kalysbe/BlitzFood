...
//class IndexAndCapitalization {
    ...
    public function buildLastQuery() {
        $this->index_query = "SELECT `date`,`index` FROM `mod_cap_index`
             GROUP BY `date`,`index` ORDER BY `date` DESC LIMIT 15";
...
#---------------------------------------------------------------------------	
	$chart = new D3Charts("Line",700,350);			# Create Line Chart
	$chart->setChartParam($lineParam);
	
	$pdata = $icap->getIndexArray();
	$keys = array_keys($pdata[0]);
	$data = array();
	
	for($i=0; $i < count($pdata); $i++)
	{
		$data[$i]['x'] = '"' . $pdata[$i][$keys[1]] . '"';
		$data[$i]['y'] = $pdata[$i][$keys[0]];
	}
	
	$chart->addChartData($data);

	
    $Controller->appendVariable("IndexChart", array_reverse($data));
#-----------------------------------------------------------------------------	
	$chart = new D3Charts("Area",700,300);			# Create Area Chart
	$chart->setChartParam($areaParam);
	
	$pdata = $icap->getCapitalizationArray();
	$keys = array_keys($pdata[0]);
	$data = Array();
	...
	$Controller->appendVariable("CapitalizationChart", array_reverse($data));
#--------------------------------------------------------------------------------   
	//--------------------------------------
