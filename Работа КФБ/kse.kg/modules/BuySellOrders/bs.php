...
function order_send($operation, $emitent, $price, $amount, $comment) {
    ...
    $message = '
        <center><h3>Новая заявка на сайте КФБ</h3></center>
		<hr>
        На сайте Кыргызской Фондовой Биржи (http://www.kse.kg)
        была подана заявка на операции с ценными бумагами.<br><br>
    ...
    $transport = Swift_SmtpTransport::newInstance('192.168.0.38', 25);
    //Create the Mailer using your created Transport
    $mailer = Swift_Mailer::newInstance($transport);
	 if (isset($_POST['turing'])) {
		 if (($_POST['turing'] != $_SESSION['secure_code'])){
			echo "<script>window.history.back();alert('Неверный код подтверждения!');</script>";
			unset($_SESSION['ByeSellOrders']);
			die();
	 }}
	else if (!isset($_POST['turing'])) {
		echo "<script>window.history.back();alert('Неверный код подтверждения!');</script>";
		unset($_SESSION['ByeSellOrders']);
		die();
	}
    ...
    $message = Swift_Message::newInstance('Новая заявка на сайте WWW.KSE.KG')
      ...
      ->setTo(array('admin@kse.kg',
        // 'bnc@bnc.kg',
        // 'senti@senti.kg',
       //  'dankokatya@rambler.ru',
        //  'orientcapital@rambler.ru',
        //  'bbb_123@mail.ru',
       //   'bank@amanbank.kg',
		//  'office@niet-araket.kg',
		//  'bnc.finance.ib@gmail.com',
		//  'pansher6666@mail.ru',
		//  'director@aic.kg',
		//  'ato.finance.consult@gmail.com',
		//  'info@kls-securities.com',
		//  'usubakunov.e@escapital.biz',
		//  'svost@infotel.kg',
		//  'zaman@elcat.kg',
		//  'sakimbaev@yandex.ru',
		//  'arpa@mail.kg',
...
function BuySellOrders_loadController() {
    ...
    if (isset($_POST["doSendMessage"])) {
		...
		if ($operation=='Покупка') {$operation='buy';}
		if ($operation=='Продажа') {$operation='sell';}
        ...
        $Controller->appendVariable("SendResult", "
            Ваша заявка была успешно принята. В ближайшее время с Вами свяжутся представители брокерских компаний.
        ");
    }
